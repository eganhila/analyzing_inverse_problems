...
for i in range(c.N_blocks):
    ...
    if c.use_permutation:
        nodes.append(Node([nodes[-1].out0], permute_layer,
...
# model = Ff.SequenceINN(c.ndim_total)
# for k in range(2):
#     model.append(Fm.AllInOneBlock, subnet_constructor=subnet_fc,
#                  permute_soft=True)
model.to(c.device)
...
optim = torch.optim.Adam(params_trainable, lr=c.lr_init,
                         betas=c.adam_betas, eps=1e-6, weight_decay=c.l2_weight_reg)
